...
from aiogram.utils import executor
import requests
from secret_info import API_TOKEN, vafli_group_id, open_weather_token
...
# СТАТИСТИКА
egor, misha, olga, dascha = 0, 0, 0, 0
...
async def message_from_users(message: Message):
    global egor, misha, olga, dascha
    if message.from_user.id == '1330610087':
        egor += 1
    if message.from_user.id == '1766360104':
        misha += 1
    if message.from_user.id == '1071828913':
        olga += 1
    if message.from_user.id == '313658535':
        dascha += 1
